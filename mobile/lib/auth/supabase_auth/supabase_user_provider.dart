...
class OtolithicsSupabaseUser extends BaseAuthUser {
  OtolithicsSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> otolithicsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = OtolithicsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
