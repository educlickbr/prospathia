...
const semestreOptions = computed(() => [
    { label: getAnoSemestre(undefined, -1), value: getAnoSemestre(undefined, -1) },
...
const filters = ref({
    pcd: null as string | null, // 'sim', 'nao'
    laudo: null as boolean | null, // true, false
    deferimento: null as string | null // 'Deferido', 'Indeferido', 'Aguardando'
});
...
const hasActiveFilters = computed(() => {
    return !!(filters.value.pcd || filters.value.laudo !== null || filters.value.deferimento);
});
...
const fetchCandidatos = async (page = 1) => {
    ...
    isLoading.value = true;
    try {
        // Prepare extra filters (Deferimento)
        const extraFilters = [];
        if (filters.value.deferimento) {
            let deferimentoQuestionId = null;
            const areaKey = areaMap[currentArea.value];
            
            if (areaKey === 'regulares') deferimentoQuestionId = ID_DEFERIMENTO_REGULARES;
            if (areaKey === 'cursos_livres') deferimentoQuestionId = ID_DEFERIMENTO_LIVRES;
        ...
        const response: any = await $fetch('/api/selecao/candidatos', {
            params: {
                ...
                filtros: JSON.stringify(extraFilters),
                pcd: filters.value.pcd || null,
                laudo: filters.value.laudo, // Pass boolean or null directly
        ...
        <div class="bg-div-15 rounded-xl p-4 md:p-6 mb-6 border border-secondary/5">
            ...
            <div class="flex flex-col md:flex-row gap-3 items-end">
                ...
                <button 
                    ...
                    class="h-12 px-4 rounded-lg flex items-center gap-2 border transition-colors shrink-0"
                    :class="showFilters || hasActiveFilters ? 'bg-primary/10 border-primary text-primary' : 'bg-[#16161E] border-white/10 text-gray-400 hover:text-white'"
                >
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path>
                    </svg>
                    <span v-if="hasActiveFilters" class="absolute top-[-4px] right-[-4px] w-2.5 h-2.5 bg-primary rounded-full ring-2 ring-[#0F0F13]"></span>
                </button>
            </div>
            
            <!-- EXPANDABLE FILTER PANEL -->
             <div v-show="showFilters" class="mt-4 pt-4 border-t border-white/5 grid grid-cols-1 md:grid-cols-4 gap-4 items-end animate-in fade-in slide-in-from-top-2 duration-200">
                <!-- PCD -->
                <div>
                    <label class="block text-xs text-secondary-600 mb-1.5">PCD</label>
                    <BaseSelect 
                        v-model="filters.pcd" 
                        :options="pcdOptions"
                        label-key="label"
                ...
                <!-- Laudo -->
                <div>
                    <label class="block text-xs text-secondary-600 mb-1.5">Laudo Médico</label>
                    <BaseSelect 
                        v-model="filters.laudo" 
                ...
                <!-- Deferimento (Only specific areas) -->
                <div>
                    <label class="block text-xs text-secondary-600 mb-1.5" :class="{'opacity-50':areaMap[currentArea] === 'extensao'}">Deferimento</label>
                    <BaseSelect 
                        v-model="filters.deferimento" 
                        :options="deferimentoOptions"
                        label-key="label"
                        value-key="value"
                        :disabled="areaMap[currentArea] === 'extensao'"
                    />
                </div>

                <!-- Clear Button -->
                <div v-if="hasActiveFilters" class="flex justify-end md:justify-start">
                     <button 
                        @click="clearFilters"
