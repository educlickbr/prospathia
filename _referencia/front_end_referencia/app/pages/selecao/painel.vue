...
const handleExcelExport = async () => {
    ...
    try {
        const { data, error } = await (client.rpc as any)('nxt_get_exportacao_excel_seletivo', {
            p_ano_semestre: anoSemestre.value,
            p_area: currentAreaValue.value,
            p_id_turma: null,
            p_tipo_candidatura: 'estudante'
        ...
        // Data format: data.dynamic_columns = [{ id: '...', label: '...' }, ...]
        const dynamicCols = (data.dynamic_columns || []).map((q: any) => ({
            header: q.label,
            key: q.label, // Using label as key since answers object uses labels now
            width: 30
        }));
        ...
        const rows = data.data.map((item: any) => {
            ...
            const flatRow: any = {
                ...
                status: item['Status'],
                // Spread answers directly since keys match headers
                ...item.respostas
            };
            return flatRow;
        });
        ...
        headerRow.fill = {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: 'FFfd0054' } // Primary Color (approximate)
        };

        // 4. Generate & Save
        const buffer = await workbook.xlsx.writeBuffer();
        const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        saveAs(blob, `Candidatos_${currentArea.value}_${anoSemestre.value}.xlsx`);
...
<template>
    <NuxtLayout name="base">
        ...
        <div class="bg-transparent md:bg-div-15 rounded-none md:rounded-xl p-0 md:p-8">
            ...
            <!-- LOADING -->
            <!-- LOADING -->
            <div v-if="isContentLoading" class="flex justify-center py-20">
            ...
            <div class="bg-div-15 rounded-xl p-5 border border-secondary/10">
                ...
                <div class="space-y-2">
                    ...
                    <template v-if="currentArea === 'Regulares'">
                        ...
                        <button @click="handleExcelExport" :disabled="isExcelLoading" class="w-full bg-[#16161E] hover:bg-white/5 border border-white/5 text-secondary hover:text-white text-xs font-bold py-3 px-4 rounded-lg transition-colors flex items-center justify-between group disabled:opacity-50 disabled:cursor-not-allowed">
                            <div class="flex flex-col items-start">
                                <span>{{ isExcelLoading ? 'Baixando...' : 'Baixar Excel' }}</span>
                                <span class="text-[10px] opacity-60 font-normal">{{ currentArea }}</span>
                            </div>
                            <div v-if="isExcelLoading" class="animate-spin h-4 w-4 border-t-2 border-primary rounded-full"></div>
                            <svg v-else class="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        </button>
                    </template>
